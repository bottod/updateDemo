...
uint16_t table_crc16_x25(uint16_t crc,uint8_t  *ptr,uint16_t len) // 字节查表法求CRC
{
    ...
    while(len--!=0)
    {
        invert_uint8(*ptr,&tmpValue);
        da=(uint8_t) (crc/256); // 以8 位二进制数暂存CRC 的高8 位
        crc<<=8; // 左移8 位
        crc^=crc16tab[da^tmpValue]; // 高字节和当前数据XOR 再查表
        ptr++;
    }
